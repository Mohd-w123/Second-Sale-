      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-[40px] border border-gray-100 p-8 sm:p-12 shadow-sm space-y-12">
          ...
          <div className="flex flex-wrap gap-y-2 gap-x-8 text-sm font-bold">
             ...
             <p className="text-[#2563EB]">Pickup Date: <span className="font-black">{pickupDateStr} - {order.pickup?.timeSlot}</span></p>
          </div>

          <div className="border-t border-gray-50 pt-10">
            <h3 className="text-lg font-black text-[#111827] mb-10">Order Status: <span className="text-[#2563EB]">Generated</span></h3>
            
            {/* Progress Bar */}
            <div className="relative pt-12 pb-8">
              <div className="absolute top-[60px] left-0 right-0 h-1 bg-gray-100 rounded-full" />
              <div 
                className="absolute top-[60px] left-0 h-1 bg-[#2563EB] rounded-full transition-all duration-500" 
                  ...
                  return (
                    <div key={step.label} className="flex flex-col items-center">
                      ...
                      <div className={`w-6 h-6 rounded-full border-4 border-white shadow-sm relative z-10 transition-all duration-300
                        ${isActive ? 'bg-[#2563EB] scale-125' : 'bg-gray-200'}`} />
                    </div>
                  );
                })}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 pt-10 border-t border-gray-50">
            <div className="space-y-6">
              <div>
                ...
                <p className="text-xs font-bold text-gray-400 mb-3">Payment Type : <span className="text-[#2563EB] capitalize">{order.pickup?.paymentMethod}</span></p>
                {['placed', 'scheduled'].includes(order.status) && (
                  <button 
                    onClick={() => setShowUpdatePayment(true)}
                    className="text-xs font-black text-[#2563EB] border border-[#2563EB] rounded-xl px-4 py-2 hover:bg-[#2563EB] hover:text-white transition-all shadow-sm"
                  >
            ...
            <div className="space-y-6">
               <div>
                  ...
                  <p className="text-xs font-bold text-gray-400 mb-4">Pickup Status: <span className="text-[#2563EB]">Order Generated</span></p>
                    ...
                    {['placed', 'scheduled'].includes(order.status) && (
                      <>
                        <button 
                          ...
                          className="flex items-center gap-2 bg-[#E6F4FF] text-[#2563EB] px-6 py-2.5 rounded-xl font-black text-xs hover:bg-[#2563EB] hover:text-white transition-all shadow-sm"
                        >
        ...
        <div className="space-y-6">
              ...
              {dates.map(date => (
                <button
                  ...
                  className={`py-3 rounded-xl border text-xs font-black transition-all
                    ${selectedDate === date ? 'bg-[#2563EB] text-white border-[#2563EB]' : 'bg-gray-50 text-gray-500 border-gray-100 hover:border-gray-200'}`}
                >
              ...
              {slots.map(slot => (
                <button
                  ...
                  className={`w-full py-4 px-6 rounded-2xl border text-sm font-black transition-all text-left flex items-center justify-between
                    ${selectedSlot === slot ? 'bg-[#E6F4FF] text-[#2563EB] border-[#2563EB]' : 'bg-gray-50 text-gray-500 border-gray-100 hover:border-gray-200'}`}
                >
                  {slot}
          ...
          <div className="flex gap-4 pt-4">
            ...
            <button 
              ...
              className="flex-2 bg-[#2563EB] text-white font-black py-4 px-8 rounded-2xl hover:bg-black transition-all disabled:opacity-50"
            >
  ...
  return (
    <div className="flex items-center justify-between group cursor-pointer">
      <div className="flex items-center gap-3">
        <div className="w-6 h-6 rounded-full border border-gray-100 flex items-center justify-center text-gray-400 group-hover:text-[#2563EB] transition-colors">
      ...
      <svg className="text-gray-300 group-hover:text-[#2563EB] group-hover:translate-x-0.5 transition-all" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M7 17l10-10M7 7h10v10"/></svg>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-2xl rounded-[40px] shadow-2xl p-10 max-h-[90vh] overflow-y-auto no-scrollbar">
        ...
        <div className="space-y-4">
          {/* UPI */}
          <div className={`rounded-[32px] border-2 p-6 transition-all ${paymentType === 'upi' ? 'border-[#2563EB] bg-[#E6F4FF]' : 'border-gray-50 bg-white hover:border-gray-200 cursor-pointer'}`}>
            <div className="flex items-center gap-6" onClick={() => setPaymentType('upi')}>
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm shrink-0 border ${paymentType === 'upi' ? 'bg-[#2563EB] text-white border-[#2563EB]' : 'bg-white text-gray-500 border-gray-100'}`}>
              ...
              <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${paymentType === 'upi' ? 'border-[#2563EB] bg-[#2563EB]' : 'border-gray-300'}`}>
                {paymentType === 'upi' && <div className="w-2.5 h-2.5 bg-white rounded-full" />}
              </div>
            </div>
            
            {paymentType === 'upi' && (
              <div className="mt-6 pt-6 border-t border-[#2563EB]/20">
                <div className="space-y-3 mb-4">
                  {user?.paymentMethods?.filter(pm => pm.type === 'upi').map(pm => (
                    <label key={pm._id} className="flex items-center gap-3 cursor-pointer p-3 rounded-xl hover:bg-[#2563EB]/5 transition-colors">
                      <input 
                        type="radio" 
                        name="upiId" 
                ...
                <button 
                  ...
                  className="text-sm font-black text-[#2563EB] hover:text-[#1D4ED8] flex items-center gap-2"
                >
          ...
          {/* Bank */}
          <div className={`rounded-[32px] border-2 p-6 transition-all ${paymentType === 'bank' ? 'border-[#2563EB] bg-[#E6F4FF]' : 'border-gray-50 bg-white hover:border-gray-200 cursor-pointer'}`}>
            <div className="flex items-center gap-6" onClick={() => setPaymentType('bank')}>
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm shrink-0 border ${paymentType === 'bank' ? 'bg-[#2563EB] text-white border-[#2563EB]' : 'bg-white text-gray-500 border-gray-100'}`}>
              ...
              <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${paymentType === 'bank' ? 'border-[#2563EB] bg-[#2563EB]' : 'border-gray-300'}`}>
          ...
          {/* Cash */}
          <div 
            className={`rounded-[32px] border-2 p-6 transition-all ${paymentType === 'cash' ? 'border-[#2563EB] bg-[#E6F4FF]' : 'border-gray-50 bg-white hover:border-gray-200 cursor-pointer'}`}
            onClick={() => setPaymentType('cash')}
          >
            <div className="flex items-center gap-6">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm shrink-0 border ${paymentType === 'cash' ? 'bg-[#2563EB] text-white border-[#2563EB]' : 'bg-white text-gray-500 border-gray-100'}`}>
              ...
              <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${paymentType === 'cash' ? 'border-[#2563EB] bg-[#2563EB]' : 'border-gray-300'}`}>
        ...
        <button 
          ...
          className="w-full mt-8 bg-[#2563EB] text-white font-black py-5 rounded-[24px] hover:bg-[#1D4ED8] transition-all shadow-xl shadow-blue-100 disabled:opacity-50"
        >
    ...
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-md rounded-[40px] shadow-2xl p-10 max-h-[90vh] overflow-y-auto no-scrollbar">
        ...
        <div className="flex items-center gap-4 mb-10">
          <div className="w-12 h-12 bg-[#E6F4FF] rounded-2xl flex items-center justify-center text-[#2563EB]">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {type === 'upi' ? (
            <div>
              ...
              <input 
                ...
                className="w-full bg-white border-2 border-gray-100 rounded-2xl px-6 py-4 text-sm font-bold focus:outline-none focus:border-[#2563EB] transition-all"
                required
              />
            </div>
          ) : (
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full mt-6 bg-[#2563EB] text-white font-black py-5 rounded-[24px] hover:bg-[#1D4ED8] transition-all shadow-xl shadow-blue-100 flex items-center justify-center gap-2"
          >
