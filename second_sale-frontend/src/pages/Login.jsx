...
function PhoneStep({ onNext, loading, error }) {
  ...
  return (
    <>
      <div className="inline-flex items-center gap-2 bg-[#087F8C]/5 text-[#087F8C] text-[10px] font-bold tracking-widest uppercase px-3 py-1 rounded-full mb-6 border border-gray-100">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          ...
          <div className="relative flex items-center">
            ...
            <input
              ...
              inputMode="numeric"
              maxLength={10}
              placeholder="70451 80009"
              value={phone}
              onChange={(e) => setPhone(e.target.value.replace(/\D/g, "").slice(0, 10))}
              autoComplete="tel"
              className="w-full pl-[4.5rem] pr-4 py-3 border-[1.5px] border-gray-200 rounded-xl text-sm font-sans text-text-primary outline-none bg-gray-50 focus:border-[#087F8C] focus:ring-4 focus:ring-[#087F8C]/10 focus:bg-white transition-all tracking-widest"
            />
          </div>
        </div>
        ...
        <button
          ...
          className="w-full btn-gradient text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-3 shadow-lg shadow-[#087F8C]/30 hover:shadow-xl transition-all hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0"
        >
...
function OtpStep({ phone, onVerify, onBack, loading, error, onResend }) {
  ...
  return (
    <>
      <div className="inline-flex items-center gap-2 bg-[#087F8C]/5 text-[#087F8C] text-[10px] font-bold tracking-widest uppercase px-3 py-1 rounded-full mb-6 border border-gray-100">
        <div className="w-1.5 h-1.5 bg-[#087F8C] rounded-full animate-pulse" />
        OTP Sent
      </div>
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <button
          type="button"
          onClick={onBack}
          className="flex items-center gap-1 text-[11px] font-bold text-[#087F8C] hover:underline bg-transparent border-none cursor-pointer p-0"
        >
          <EditIcon /> Edit
        </button>
      </div>

      <div className="bg-[#E8F6F7] border border-[#087F8C]/20 text-[#087F8C] px-3.5 py-2 rounded-xl text-xs font-semibold mb-6 flex items-center gap-2">
            ...
            {otp.map((digit, index) => (
              <input
                ...
                className={`w-full aspect-square text-center text-xl font-black border-[2px] rounded-xl outline-none transition-all bg-gray-50 focus:bg-white
                  ${digit ? "border-[#087F8C] text-[#087F8C] bg-[#087F8C]/5" : "border-gray-200 text-text-primary"}
                  focus:border-[#087F8C] focus:ring-4 focus:ring-[#087F8C]/10`}
              />
            ))}
          </div>
        </div>
    ...
    <div className="text-center mt-6">
      {canResend ? (
        <button
          ...
          className="text-sm font-bold text-[#087F8C] hover:underline bg-transparent border-none cursor-pointer"
        >
          Resend OTP
        </button>
      ) : (
...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-[90vh] bg-gradient-to-br from-[#087F8C]/5 via-white to-[#087F8C]/5 flex items-center justify-center p-4 sm:p-8">
      <SEOHead
        ...
        noindex
      />
      <div className="w-full max-w-[440px] bg-white rounded-[32px] shadow-2xl shadow-[#087F8C]/10 border border-gray-100 p-8 sm:p-12 animate-in fade-in zoom-in-95 duration-300">
            ...
            return (
              <div key={label} className="flex items-center gap-2">
                <div className={`flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider transition-all
                  ${isActive ? "text-[#087F8C]" : isDone ? "text-green-500" : "text-gray-300"}`}>
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-black
                    ${isActive ? "bg-[#087F8C] text-white" : isDone ? "bg-green-500 text-white" : "bg-gray-100 text-gray-400"}`}>
                    {isDone ? "✓" : i + 1}
                  </div>
                  {label}
                </div>
                {i < 1 && (
                  <div className={`w-8 h-px transition-all ${isDone || isActive ? "bg-[#087F8C]/30" : "bg-gray-100"}`} />
                )}
              </div>
            );
          })}
        ...
        <p className="text-center text-sm text-text-muted font-medium">
          ...
          <Link to="/signup" className="text-[#087F8C] font-bold hover:underline">Sign up</Link>
        </p> */}
        ...
        <div className="flex justify-center gap-5 mt-10">
          {["SSL Secure", "No Spam", "Free Pickup"].map((t) => (
            <div key={t} className="flex items-center gap-1.5 text-[10px] font-bold text-gray-400 uppercase tracking-tight">
              <div className="w-1.5 h-1.5 bg-[#087F8C]/40 rounded-full" />
              {t}
            </div>
          ))}
        </div>
