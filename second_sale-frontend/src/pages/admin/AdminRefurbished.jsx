                    ...
                    {orders.map((o) => (
                      <tr key={o._id || o.orderId} className="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td className="py-3.5 px-4">
                          ...
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-md font-bold text-[10px] uppercase mt-1 block w-fit ${
                            o.payment?.status === 'confirmed' ? 'bg-emerald-100 text-emerald-800' :
                            o.payment?.status === 'failed' ? 'bg-rose-100 text-rose-800' :
                            o.payment?.status === 'refunded' ? 'bg-purple-100 text-purple-800' :
                            'bg-amber-100 text-amber-800'
                          }`}>
                            {o.payment?.status === 'confirmed' ? 'Paid / Verified' :
                             o.payment?.status === 'failed' ? 'Failed' :
                             o.payment?.status === 'refunded' ? 'Refunded' : 'Pending'}
                          </span>
                          {o.payment?.transactionId && (
                            <span className="block text-[10px] text-slate-400 font-mono mt-0.5">
                              TXN: {o.payment.transactionId}
                            </span>
                          )}
                        </td>
                        ...
                        <td className="py-3.5 px-4">
                          ...
                          {o.deliveryDetails?.trackingNumber && (
                            <span className="block text-[10px] text-slate-400 font-mono mt-1">
                              AWB: {o.deliveryDetails.trackingNumber}
                            </span>
                          )}
                          {o.deliveryDetails?.courierPartner && (
                            <span className="block text-[10px] text-slate-500 font-medium">
                              Via {o.deliveryDetails.courierPartner}
                            </span>
                          )}
                        </td>
            ...
            <form onSubmit={handleSaveOrderStatus} className="space-y-3.5">
              ...
              <div>
                ...
                >
                  ...
                  <option value="confirmed">Confirmed (Diagnostics Passed)</option>
                  <option value="packed">Packed in Eco-Box</option>
              ...
              <div>
                ...
                <select
                  value={trackingData.paymentStatus}
                  onChange={(e) => setTrackingData({ ...trackingData, paymentStatus: e.target.value })}
                  className="w-full px-3.5 py-2 text-xs font-bold border border-slate-200 rounded-xl bg-slate-50"
                >
                  <option value="pending">Pending (Awaiting Payment / COD)</option>
                  <option value="confirmed">Paid / Verified</option>
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. UPI Ref / Bank TXN / COD-PAID"
                  value={trackingData.transactionId}
                  onChange={(e) => setTrackingData({ ...trackingData, transactionId: e.target.value })}
                  className="w-full px-3.5 py-2 text-xs font-mono border border-slate-200 rounded-xl bg-slate-50"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  rows="2"
                  placeholder="Optional internal remarks for this order..."
                  value={trackingData.notes}
                  onChange={(e) => setTrackingData({ ...trackingData, notes: e.target.value })}
                  className="w-full px-3.5 py-2 text-xs border border-slate-200 rounded-xl bg-slate-50"
                />
              </div>
