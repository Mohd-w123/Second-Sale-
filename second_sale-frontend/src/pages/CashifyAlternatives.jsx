...
const FAQ = {
  ...
  a: 'Top Cashify alternatives for selling old devices in India include SecondSale (instant quotes and free pickup), OLX (peer-to-peer listings), and exchange programs on Amazon or Flipkart when upgrading to a new device.',
};
  ...
  return (
    <>
      <SEOHead
        ...
        description="Looking for Cashify alternatives? Compare SecondSale, OLX, and exchange programs for selling old phones, laptops and tablets in India with free pickup and instant cash."
      ...
      <div className="max-w-[900px] mx-auto px-4 py-12 sm:py-20">
        ...
        <p className="text-gray-500 mb-10 leading-relaxed">
          If you're exploring options beyond Cashify to sell your old phone, laptop, or tablet, here are the leading alternatives in India — including SecondSale, which offers instant quotes and free doorstep pickup nationwide.
        </p>
        ...
        <div className="space-y-6">
          {ALTERNATIVES.map((alt) => (
            <article
              key={alt.name}
              className={`rounded-2xl p-6 border ${alt.highlight ? 'border-[#087F8C] bg-[#E8F6F7] shadow-md' : 'border-gray-100 bg-white shadow-sm'}`}
            >
              <div className="flex items-center gap-3 mb-3">
                <h2 className="text-xl font-black text-gray-900">{alt.name}</h2>
                {alt.highlight && (
                  <span className="text-xs font-bold bg-[#087F8C] text-white px-2 py-1 rounded-full">
                    Recommended
                  </span>
                )}
              </div>
              ...
              <ul className="text-sm text-gray-600 space-y-1 mb-4">
                {alt.pros.map((pro) => (
                  <li key={pro} className="flex items-center gap-2">
                    <span className="text-[#087F8C] font-bold">✓</span> {pro}
                  </li>
                ))}
              </ul>
              {alt.url && (
                <Link to={alt.url} className="text-[#087F8C] font-bold text-sm hover:underline">
