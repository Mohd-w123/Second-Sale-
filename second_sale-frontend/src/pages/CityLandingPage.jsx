  ...
  return (
    <>
      <SEOHead
        ...
        description={`Sell your old mobile phone in ${city.name}, ${city.state} with SecondSale. Free doorstep pickup, instant online quote, and secure payment across ${city.name}.`}
        path={path}
        schema={schema}
      />
      <div className="max-w-[900px] mx-auto px-4 py-12 sm:py-20">
        ...
        <p className="text-gray-500 mb-6 leading-relaxed">
          SecondSale offers free doorstep pickup in {city.name}, {city.state}. Sell your used smartphone online without visiting a shop — get an instant quote, schedule pickup from home, and receive payment via UPI or bank transfer after verification.
        </p>
        ...
        <div className="flex flex-wrap gap-3 mb-10">
          {TOP_BRANDS.map((brand) => (
            <Link
              key={brand.name}
              to={brand.path}
              className="bg-[#F7FAFF] border border-gray-200 rounded-xl px-5 py-3 text-sm font-bold text-gray-700 hover:border-[#087F8C] hover:text-[#087F8C] transition-colors no-underline"
            >
              Sell {brand.name}
            </Link>
          ))}
        ...
        <div className="grid sm:grid-cols-3 gap-4 mb-8">
          {[
            ...
            <div key={s.step} className="bg-white border border-gray-100 rounded-xl p-5">
              <div className="w-8 h-8 bg-[#087F8C] text-white rounded-full flex items-center justify-center font-black text-sm mb-3">
                {s.step}
              </div>
        ...
        <Link
          ...
          className="inline-flex btn-gradient text-white font-bold px-6 py-3 rounded-xl shadow-md shadow-[#087F8C]/20 transition-colors no-underline"
        >
