...
export default function EarbudsModelSelectionPage() {
  ...
  useEffect(() => {
    if (!brand) return;
    deviceService.getModels(brand, "earbuds")
      .then(res => {
        setModels(res.data || []);
        setLoading(false);
      })
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-4 sm:px-8 pt-5 sm:pt-8 pb-12 sm:pb-20">
      <SEOHead
        title={`Sell Old ${brandName} Earbuds Online — Instant Cash | SecondSale`}
        description={`Sell your used ${brandName} earbuds and AirPods online with SecondSale. Instant quotes, free doorstep pickup, and secure payment across India.`}
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-3xl border border-dashed border-slate-200 p-8">
          ...
          <p className="text-base sm:text-lg font-bold text-slate-700 mb-1">No {brandName} models found</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
          {filtered.map(model => (
            <Link
              key={model.slug}
              to={`/sell-earbuds/${brand}/${model.slug}`}
            ...
            >
              <div className="w-full h-40 sm:h-48 rounded-2xl bg-slate-50 flex items-center justify-center p-4 mb-4 group-hover:scale-105 transition-transform overflow-hidden">
                <img
                  src={model.imageUrl || "https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/MQD83?wid=800&hei=800&fmt=jpeg&qlt=90"}
                  alt={model.modelName}
