...
import { Gamepad2, ShieldCheck, Zap, Truck, Star } from "lucide-react";

export default function GamingModelDetailsPage() {
  ...
  if (!device) {
    return (
      <div className="max-w-md mx-auto py-24 text-center px-4">
        ...
        <h2 className="text-xl font-bold text-slate-800 mb-2">Console Not Found</h2>
        <p className="text-sm text-slate-500 mb-6">We couldn&apos;t find this gaming console in our catalog.</p>
        <Link
          to="/sell-old-gaming-consoles"
  ...
  const breadcrumbItems = [
    { label: "Home", href: "/" },
    { label: "Gaming Console", href: "/sell-gaming/brand" },
    { label: brandName, href: `/sell-gaming/${brand}` },
    { label: device.modelName },
  ];

  const handleStartQuiz = () => {
    const variantParam = selectedVariant?.storage ? `?variant=${encodeURIComponent(selectedVariant.storage)}` : "";
    navigate(`/sell-gaming/${brand}/${slug}/quiz${variantParam}`);
  };
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-4 sm:px-8 pt-5 sm:pt-8 pb-12 sm:pb-20">
      <SEOHead
        ...
        description={`Sell your used ${device.modelName} console online for the highest cash price. Get an instant quote, free doorstep pickup, and instant bank/UPI payment with SecondSale.`}
        canonicalUrl={`/sell-gaming/${brand}/${slug}`}
      />
      ...
      <div className="bg-white border border-slate-100 rounded-[32px] p-6 sm:p-10 lg:p-12 shadow-sm max-w-5xl mx-auto flex flex-col md:flex-row gap-8 lg:gap-12 items-center mt-6">
        ...
        <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left w-full">
          <div className="w-full h-64 sm:h-80 rounded-3xl bg-slate-50 flex items-center justify-center p-6 mb-6 border border-slate-100/80">
            <img
              src={device.imageUrl || "https://m.media-amazon.com/images/I/51051FiD9UL._SL1000_.jpg"}
          ...
          <div className="flex items-center gap-1.5 text-amber-400 mb-2">
            ...
            <span className="text-xs font-bold text-slate-500 ml-1.5">4.9 / 5 (3,100+ reviews)</span>
          </div>
        ...
        <div className="flex-1 w-full flex flex-col">
          ...
          {device.variants && device.variants.length > 1 && (
            <div className="mb-6">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-2">
                Select Edition / Storage Capacity
              </label>
          ...
          <div className="bg-slate-50 rounded-2xl p-5 border border-slate-200/80 mb-6">
            ...
            <p className="text-[11px] text-slate-500 mt-2">
              * Exact cash value calculated based on console condition, accessories &amp; game CDs.
            </p>
          </div>
