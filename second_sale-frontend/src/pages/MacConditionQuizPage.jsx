            ...
            <div className="flex-1 space-y-8">
              ...
              <div className="bg-white rounded-[40px] border border-gray-100 p-12 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-10">
                   ...
                   <EvaluationDetailRow label="Accessories" value={accessories.length > 0 ? accessories.join(', ') : 'None'} color="#2563EB" />
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 space-y-8">
                 ...
                 {AGE_OPTIONS.map(opt => (
                   <button 
                     ...
                     className={`flex items-center gap-4 px-6 py-5 rounded-2xl border-[1.5px] font-semibold text-left transition-all w-full
                       ${age === opt.key ? 'border-[#2563EB] bg-[#2563EB]/5 text-[#2563EB]' : 'border-gray-100 text-gray-700 bg-white hover:border-gray-200'}`}
                   >
            ...
            <EvaluationStepCard title="3. Any functional problems?" active={currentStep >= 2}>
               <div className="grid grid-cols-2 gap-4">
                 {['Yes', 'No'].map(v => (
                   <button key={v} onClick={() => handleFunctionalSelect(v.toLowerCase())} className={`py-5 rounded-2xl border-[1.5px] font-black transition-all ${((functionalIssues === 'yes' && v === 'Yes') || (functionalIssues === 'no' && v === 'No')) ? 'border-[#2563EB] bg-[#2563EB]/5 text-[#2563EB]' : 'border-gray-100 text-gray-500 hover:border-gray-200'}`}>{v}</button>
                 ))}
               </div>
            </EvaluationStepCard>
            <EvaluationStepCard title="4. Any screen issues?" active={currentStep >= 3}>
               <div className="grid grid-cols-2 gap-4">
                 {['Yes', 'No'].map(v => (
                   <button key={v} onClick={() => handleScreenSelect(v.toLowerCase())} className={`py-5 rounded-2xl border-[1.5px] font-black transition-all ${((screenIssues === 'yes' && v === 'Yes') || (screenIssues === 'no' && v === 'No')) ? 'border-[#2563EB] bg-[#2563EB]/5 text-[#2563EB]' : 'border-gray-100 text-gray-500 hover:border-gray-200'}`}>{v}</button>
                 ))}
               </div>
            </EvaluationStepCard>
            <EvaluationStepCard title="5. Any body damage?" active={currentStep >= 4}>
               <div className="grid grid-cols-2 gap-4">
                 {['Yes', 'No'].map(v => (
                   <button key={v} onClick={() => handleBodySelect(v.toLowerCase())} className={`py-5 rounded-2xl border-[1.5px] font-black transition-all ${((bodyIssues === 'yes' && v === 'Yes') || (bodyIssues === 'no' && v === 'No')) ? 'border-[#2563EB] bg-[#2563EB]/5 text-[#2563EB]' : 'border-gray-100 text-gray-500 hover:border-gray-200'}`}>{v}</button>
                 ))}
               </div>
            </EvaluationStepCard>
          </div>
          <div className="w-full lg:w-[450px]">
            <div className="sticky top-8 space-y-8">
              <div className="bg-[#2563EB]/5 rounded-[32px] p-8 border border-[#2563EB]/20 shadow-sm flex items-center justify-between">
                 <div><p className="text-[#2563EB] text-xs font-black uppercase tracking-widest mb-1">Estimated Value</p><p className="text-4xl font-black text-[#1D4ED8] tracking-tighter">{breakdown ? formatCurrency(currentPrice) : '₹ XX,XXX'}</p></div>
              </div>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Select Functional Issues" size="5xl">
       ...
       <div className="grid grid-cols-2 md:grid-cols-3 gap-4">{issues.map(i => (
         <button key={i.id} onClick={() => toggle(i.id)} className={`p-5 rounded-3xl border-[1.5px] flex flex-col items-center gap-3 transition-all relative ${selected.includes(i.id) ? 'border-[#2563EB] bg-[#2563EB]/5' : 'border-gray-100 bg-white'}`}>
       ...
       <button onClick={() => onFinish(selected)} className="w-full mt-8 py-4 bg-[#2563EB] text-white rounded-2xl font-black">Proceed</button>
    </Modal>
  );
}
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Screen Condition" size="3xl">
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{issues.map(i => (
         <button key={i.id} onClick={() => toggle(i.id)} className={`p-8 rounded-3xl border-[1.5px] flex flex-col items-center gap-4 transition-all ${selected.includes(i.id) ? 'border-[#2563EB] bg-[#2563EB]/5' : 'border-gray-100 bg-white'}`}>
...
function BodyIssuesModal({ isOpen, onClose, onFinish, initialList }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Body Condition" size="4xl">
       ...
       <div className="grid grid-cols-2 md:grid-cols-3 gap-4">{issues.map(i => (
         <button key={i.id} onClick={() => toggle(i.id)} className={`p-6 rounded-3xl border-[1.5px] flex flex-col items-center gap-3 transition-all ${selected.includes(i.id) ? 'border-[#2563EB] bg-[#2563EB]/5' : 'border-gray-100 bg-white'}`}>
           <div className="text-2xl">{i.icon}</div>
           <span className="text-[11px] font-black text-center leading-tight">{i.label}</span>
           <span className="text-[10px] font-black text-red-400">{i.pct}</span>
         </button>
...
function EvaluationStepCard({ title, active, children }) {
  return (<div className={`bg-white rounded-[24px] p-6 border border-gray-100 shadow-sm transition-all duration-500 ${active ? 'ring-2 ring-[#2563EB]/20 scale-[1.01]' : 'opacity-30 pointer-events-none'}`}><h3 className="text-base font-black text-[#111827] mb-4">{title}</h3>{children}</div>);
}
