...
const sortBrands = (brandsList) => {
  return [...brandsList].sort((a, b) => {
    const aIndex = SMARTWATCH_BRAND_ORDER.findIndex(
      name => name.toLowerCase() === (a.brand || a.name || "").toLowerCase()
    );
    const bIndex = SMARTWATCH_BRAND_ORDER.findIndex(
...
export default function SmartwatchBrandSelectionPage() {
  ...
  useEffect(() => {
    deviceService.getBrands("smartwatch").then(res => {
      if (res.data && res.data.length > 0) {
        setBrands(sortBrands(res.data));
      } else {
        const fallback = SMARTWATCH_BRANDS.map(b => ({
          brand: b.name,
          modelCount: b.models,
          logo: b.logo,
          color: b.color
  ...
  const getBrandLogo = (name) => {
    const b = SMARTWATCH_BRANDS.find(br => br.name.toLowerCase() === name.toLowerCase());
    return b?.logo || null;
  };
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6">
        {filteredBrands.map((b) => {
          ...
          return (
            <Link
              key={brandName}
              to={`/sell-smartwatch/${encodeURIComponent(brandName.toLowerCase())}`}
